...
| 📜 **跟着页面滚动** | 上下滚页面，标注跟着滚（方向精确，幅度可调，见[常见问题](#-常见问题)）|
| ✏️ **五种手绘工具** | 画笔、手绘箭头、荧光笔、手写便签、橡皮，六种颜色 |
| 💾 **自动记住** | 按"程序 + 窗口标题"存档，窗口关了再开，标注自动回来 |
| 🫥 **不挡你干活** | 画完按一下，鼠标穿透浮层，照常点击操作下面的窗口 |
| 🔔 **托盘常驻** | 右下角一个红色 ✎ 图标，开机自启、灵敏度都在右键菜单里 |
| 🀄 **零版权负担** | 手写体用 Windows 自带字体，不打包任何第三方素材 |

## 效果

**标注模式的工具条**（画笔 / 箭头 / 荧光笔 / 手写便签 / 橡皮 + 六色）：

<img src="docs/toolbar.png" alt="浮动工具条" width="640">

**托盘右键菜单**（开机自启、滚动灵敏度都在这）：

<img src="docs/tray.png" alt="托盘菜单" width="380">
...
**"跟着页面滚动"为什么有时对不齐？**
因为这是个**外挂在别人窗口上的浮层**，读不到浏览器 / PDF 内部滚到了第几行。
它的做法是在系统层拦截你的鼠标滚轮，把"滚了几格"换算成像素来平移标注：
**方向一定是对的**，但**幅度是估算的**——不同软件滚一格走多少像素并不一样。
如果标注比内容跑得快或慢，去托盘把「滚动灵敏度」调一档就好。

**它会拖慢电脑吗？**
后台只做一件很轻的事：每 16 毫秒对齐一次浮层位置。不标注时几乎不占资源。
...
3. **标注画布**（`overlay/`）：一个透明的置顶窗口。
   ...
   - **跟随滚动**：用 `WH_MOUSE_LL` 低级鼠标钩子在系统层截滚轮增量，按帧换算成像素平移。
     标注坐标存的是"内容坐标"（滚动=0 时的窗口坐标），所以整体平移永远一致。
