...
| ✏️ **手绘工具** | 画笔、手绘箭头、方框、荧光笔、手写便签、橡皮，**七种颜色**（含白色）|
| 📐 **测量标尺** | 拖一个框，实时显示物理像素尺寸（量按钮大小、量间距），跟着窗口走 |
| 🔍 **放大镜** | 框一块 → 抓那一刻放大成一枚玻璃镜片看细节；圆 / 方两种形状、2× / 3× / 4× 倍率；跟着窗口走 |
| 🎨 **屏幕取色** | 调色板里一支吸管，从屏幕**任意位置**吸一个颜色当画笔色（系统原生取色器，全屏可取）|
| 💾 **自动记住** | 按"程序 + 窗口标题"存档，窗口关了再开，标注自动回来 |
| 🫥 **不挡你干活** | 画完按一下，鼠标穿透浮层，照常点击操作下面的窗口 |
| 📷 **截图** | 点截图键=连窗口带标注截**整窗**；**右键截图键=框选只截选中那块**；都复制到剪贴板 + 存到 `图片\Window Annotator` |
| 🔔 **托盘常驻** | 右下角一个红色 ✎ 图标，开机自启、跟随开关都在右键菜单里 |
| 🀄 **零版权负担** | 手写体用 Windows 自带字体，不打包任何第三方素材 |

## 效果

**标注模式的工具条**（画笔 / 箭头 / 方框 / 荧光笔 / 手写便签 / 橡皮 / 测量标尺 / 放大镜 + 七色 + 屏幕取色）：

<div align="center">
<img src="docs/toolbar.png" alt="浮动工具条" width="680">
</div>

**液态玻璃工具条**（进入标注时「凝结」登场、离开时收拢消失；静止时也是一块通透的玻璃）：

<div align="center">
<img src="docs/toolbar-pop.gif" alt="工具条液态玻璃出现与消失动画" width="520">
</div>
...
| 擦掉某一笔 | 选橡皮，点它 / 从上面划过 |
| 快速切工具 | 数字键 `1`–`8` 直接切；或把滚轮切成「切工具」模式（工具条左端 ◧ 图标点 / 中键 / 右键循环:透明度 / 缩放 / 切工具）|
| 量一个元素多大 / 量间距 | 选**测量标尺**（📐）→ 拖一个框 → 实时显示物理像素 `宽 × 高 px`（标签在框外不挡内容），跟着窗口走 |
| 放大看局部细节 | 选**放大镜**（🔍）→ 框一块 → 放大成一枚可拖的玻璃镜片（那一刻的快照）。中键切倍率 2× / 3× / 4×，右键放大镜按钮切圆 / 方 |
| 用屏幕上的某个颜色来画 | 画笔模式点色点右边的**吸管** → 光标变放大镜 → 在屏幕**任意位置**点一下取色 → 该色成为画笔色，并出现在色点末尾（随时点它切回）|
| 把窗口和标注截成图 | 点工具条右侧的截图键（相机图标）截**整个窗口**（含标注），复制到剪贴板 + 存 PNG 到 `图片\Window Annotator`；托盘右键也有「打开截图文件夹」 |
| 只截选中的一块 | **右键**那个截图键 → 光标变十字 → 框一块 → 只截那块（含标注），一样复制 + 存盘 |
| 改截图保存到哪 | 托盘右键 →「截图保存位置…」选个文件夹（菜单里还会顺带显示当前存在哪个文件夹）|
...
**它会拖慢电脑吗？**
后台只做一件很轻的事：自适应对齐浮层位置——有动静时按 60fps 跟手、静止后回落省电、完全没标注时几乎休眠；滚动位置也只在真有标注的窗口在前台时才按 ~30fps 读一次。不标注时几乎不占资源。
