...
| 📷 **一键截图** | 连窗口带标注一起截成图，自动复制到剪贴板，并存一份到 `图片\Window Annotator` |
| 🔔 **托盘常驻** | 右下角一个红色 ✎ 图标，开机自启、跟随开关都在右键菜单里 |
| 🀄 **零版权负担** | 手写体用 Windows 自带字体，不打包任何第三方素材 |

## 效果

**标注模式的工具条**（画笔 / 箭头 / 方框 / 荧光笔 / 手写便签 / 橡皮 + 六色 + 屏幕取色）：
...
| 把窗口和标注截成图 | 画笔模式点工具条右侧的截图键（相机图标）—— 截当前这个窗口（含你画的标注），自动复制到剪贴板，并存一份 PNG 到 `图片\Window Annotator`；托盘右键也有「打开截图文件夹」 |
